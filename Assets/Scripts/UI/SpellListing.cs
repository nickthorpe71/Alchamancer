...
public class SpellListing : MonoBehaviour
{
    ...
    public void Click()
    {
        if(gameManager.myTurn)
            spellCaster.SubmitAttack(spell.power, spell);
        else
        {
    ...
    public void Info()
    {
        if (active)
        {
            GameManager.instance.currentSpell = spell;
            GameManager.instance.DisplayToolTip(spell.spellName, spell.toolTip, spell.power.ToString());
            CheckCost();
        }
        else
        {
